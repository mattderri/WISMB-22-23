...
\]
We can now consider $-\Tilde{H}\overline{Q}_L^i(Y_u)_{ij}u_R^j-\Tilde{H}\overline{Q}_L^i(Y_d)_{ij}d_R^j$ and focus on flavour indices. Applying CP to that term gives us:\marginnote{The phases defined in the previous transformation get reabsorbed in the $K$ matrices.}
\[
...
\marginnote{When pronouncing \textit{Jarlskog}, you might accidentally summon a demon in the extra-dimension (bad thing).}. The global symmetry given by\\
...
\left\{
\begin{aligned}
%&\text{\# physical real parameters}=2\times9\text{ real parameters}-9\text{ broken real generators}=9\\
...
\section{\href{https://en.wikipedia.org/wiki/Cabibbo-Kobayashi-Maskawa_matrix}{CKM} Matrix}\marginnote{From \cite{GT}, Section 3.}
...
\right.
\]
The absence of FCNC at tree-level is due to the fact that we have only one Higgs and we are in 4 dimensions.
...
This is suggesting us that the QCD Lagrangian containing only light flavours in the chiral limit ($m_u,m_d,m_s\to0$) is a good starting point in the discussion of low energy QCD:\marginnote{The covariant derivative acts on colour and Dirac indices only but it is independent of flavour.}
\[
...
\]
By applying \refeq{gammarelations}, we observe that the quark mass terms mix left- and right-handed fields: $\pazocal{L}_{\text{M}}=-(\overline{q}_LMq_R+\overline{q}_RMq_L)$.\\
...
\end{itemize}
\subsection{Spontaneous Symmetry Breaking}\marginnote{From \cite{SS}, Chapter 3.}
...
Hadrons do not exactly fulfill this symmetry because on top of that there is another explicit symmetry breaking, the one caused by the mass term. If we look at the spectrum in QCD, we see that there are particles with masses around the GeV and others below or around hundreds of MeV: these are $\pi, K$ and $\eta$ mesons. The fact that these are all lighter than the typical scale indicates that there is clearly a separation, they are special and get interpreted as pseudo-\href{https://en.wikipedia.org/wiki/Jeffrey_Goldstone}{Goldstone} bosons. The symmetry group G with 8+8+1=17 generators gets broken into H which has 8+1=9 generators, resulting in 8 degrees of freedom, i.e. 8 Goldstone bosons. One argument about this story is that in QCD, if we forget about the anomaly we should get also U$(1)_{\text{A}}$, expecting then 9 Goldstone bosons. The fact that we do not see 9 Goldstone bosons is an indication that the axial symmetry at the beginning was not really there.\\
...
\section{Effective Field Theories (EFT)}\marginnote{From \cite{P}, Section 2 and Section 4.}
...
\end{array}\right)
\]
The Lagrangian in \refeq{lqcd} is invariant under the following set of local SU$(3)_{\text{L}}\times$SU$(3)_{\text{R}}$ transformations:
\[
\left\{
\begin{aligned}
&q_L\to g_Lq_L\\
...
\end{align*}
At lowest order in momenta, the generating functional reduces to the classical action $S=\int d^4x\pazocal{L}$, with $\pazocal{L}$ defined in \refeq{l2} and the currents can be computed by taking the appropriate derivatives with respect to the external fields:
\[
\left\{
\begin{aligned}
...
\right.
\]
A phenomenological application of all this machinery is the charged pion decay. What we are interested in is:\marginnote{From \cite{SS}, Section 3.7.}
\[
...
\marginnote{\includegraphics[]{Images/piondecay.pdf}}
\begin{align*}
...
\]
Taking now $s=M$ and $p=0$, the $\chi$ terms in \refeq{l2} give rise to a
...
\end{align*}
Phenomenologically, the key process to measure $V_{ud}$ is the $\beta$-decay, which comes in three different forms:\marginnote{From \cite{GT}, Section 8.}
\begin{itemize}
...
\]
Move now to the decay of baryon states, i.e. $n\to pe^-\overline{\nu}_e$ which is induced by the same weak operator as before $\hat{O}$. The matrix element factorizes again into leptonic and hadronic part but unlike the meson case, both vector and axial currents contribute to this decay and the most general form for the matrix element consistent with Lorentz invariance is:\marginnote{\textit{We parametrize our ignorance into form factors}}
\begin{align*}
