...
This problem is known as the \textbf{hierarchy problem} which can be summarized as follows: we need $\mu$ of order $v$, $v$ is of order of the electroweak scale 246 GeV so we say \textit{okay, we write $\mu$ and say it is of order 246 GeV}. But then we go to one-loop level to include radiative corrections (portrayed on the side).\marginnote[-1cm]{\includegraphics{Images/higgscorrections.pdf}}
...
Given this discussion about anomalies, we ask ourselves: \textit{is baryon number a good symmetry? Does it have any anomaly?}\marginnote{From \cite{schwartz}, Section 30.4 and 30.5.1.}\\
% \[
% \left\{
% \begin{aligned}
...
% \right.
% \]
Any anomaly with one factor of SU$(2)$ or SU$(3)$ vanishes because of the properties of SU$(N)$ generators\marginnote{For SU$(N)$ we have:
\[
\{T^b,T^c\}=\frac{1}{N}\delta^{bc}\mathbb{1}
\]
When then taking the trace, the remaining $T^a$ is traceless by definition so it is automatically zero.}, this means fewer combinations to look at. Start with U$(1)^3_{\text{B}}$:
\[
D^{BBB}=\frac{1}{2}\Tr{B\{B,B\}}=\Tr{B^3}=3N_f\left[2\cdot\left(\frac{1}{3}\right)^3-\left(\frac{1}{3}\right)^3-\left(\frac{1}{3}\right)^3\right]=0
\]
The factor $3$ accounts for the colour, $N_f$ denotes the number of families, the $2$ is there to account for the fact that $Q_L$ is a doublet and the minus sign is because left-handed and right-handed fermions contribute to the anomalies with opposite signs.
There is no anomaly of baryon number under SU$(3)_{\text{C}}$U$(1)^2_{\text{B}}$:
\[
D^{Bbc}=\frac{1}{2}\Tr{B}_{\text{quarks}}\Tr{\{T^b,T^c\}}=\frac{1}{2}3N_f\left(2\cdot\frac{1}{3}-\frac{1}{3}-\frac{1}{3}\right)\delta^{bc}=0
\]
We look now at the anomaly under SU$(2)_{\text{EW}}$U$(1)^2_{\text{B}}$:
\[
D^{Bbc}=\frac{1}{2}\Tr{B}_{\text{weak doublets}}\Tr{\{T^b,T^c\}}=\frac{1}{2}2\cdot\frac{1}{3}\cdot3N_f\delta^{bc}=N_f\delta^{bc}\marginnote{In the weak doublets, only $q_L$ contributes and it is factored out because it acts on a different space.}
\]
There is no anomaly under U$(1)^2_{\text{B}}$U$(1)_{\text{Y}}$
while if we look at U$(1)^2_{\text{Y}}$U$(1)_{\text{B}}$ we find:
\begin{align*}
D^{Byy}&=\frac{1}{2}\Tr{B}_{\text{quarks}}\Tr{\{Y,Y\}}\xleftarrow[]{}\;=2Y^2\\
&=3N_f\left[\frac{1}{3}\cdot2\cdot\left(\frac{1}{6}\right)^2-\frac{1}{3}\cdot\left(\frac{2}{3}\right)^2-\frac{1}{3}\cdot\left(-\frac{1}{3}\right)^2\right]\\
...
\]
which is an accidental symmetry of the SM. In particular, B$-$L is anomalous and this is important if we want to extend the SM because it is not possible to gauge it.\marginnote[-1cm]{If you look at \cite{schwartz} Section 30.5.1 it says that B$-$L is non-anomalous because it is including the $\nu_R$ which we are not considering here.}
...
A general representation under an SU(5) transformation may be expressed in tensor notation as:\marginnote{For more details about the tensor notation, see \cite{CL}, Section 4.3.}
\[
...
\]
At Super-Kamiokande they measured $\tau(p\to\pi^0e^+)\ge1.4\cdot10^{34}$\,years resulting in $M_{\text{GUT}}\gtrsim10^{16}$\,GeV\marginnote[-2cm]{I am 0\% sure of what is written here because my notes on this part are terrible and there was someone using a drill in this exact moment so also the recording of the lecture is useless.}
\section{Custodial Symmetry}\marginnote{From \cite{custodial}.}
...
% Remember that for SO$(n)$ there are $n(n-1)/2$ generators:
% \[
% \begin{cases}
...
\]
\end{kaobox}
% The main point is that for each element of SO(4) we have two elements of SU$(2)_{\text{L}}\times$SU$(2)_{\text{R}}$. SO(4) broken to SO(3) is the same as SU$(2)_{\text{L}}\times$SU$(2)_{\text{R}}$ broken to SU$(2)_{\text{V}}$ but this is something familiar: it is the chiral symmetry breaking in QCD with 2 flavours.\\
...
In order to see corrections to the $\rho$-parameter, we can exploit the similarity with the Sigma model and rewrite the Higgs sector in terms of a new field $\pazocal{H}$:\marginnote{For more details about the Sigma model, take a look at the examples in \cite{EW}, Section 2.3.}
\[
...
The last thing left to consider is the Yukawa term, take for example the one relative to the quarks:\marginnote{For the leptons is the same thing.}
\[
...
The fact that $m_u\neq m_d$ is another explicit breaking of SU$(2)_{\text{R}}$, hence of the custodial of SO(3). In particular, the most important effects come from heavier quarks, i.e. the top and the bottom, so we can consider the corrections at one-loop to the $W$ and $Z$ bosons induced by the virtual exchange of the top and bottom quarks (look at the diagrams on the side)\marginnote{
\includegraphics[width=0.55\textwidth]{Images/wztb.pdf}
}.\\
...
Everything remains the same but we \textit{do not} need to ask for a linear realization at high energies.\marginnote{Another possible option, used in \textbf{technicolor theories}, is to completely eliminate the Higgs boson from the theory.} This means that we need to construct a Lagrangian which has the global SU$(2)\times$SU(2) symmetry broken to SU(2) and then we gauge the subgroup, we do not impose that it has to reconstruct linear multiplets. Considering all terms which are invariant under the symmetry, the most general Lagrangian we can write is:
\begin{align*}
...
The answer is no \raisebox{-\mydepth}{{\includegraphics[height=1.1\baselineskip]{Images/sadsmile.jpg}}}. Remember that the rule of this game is to have local invariance under SU$(2)\times$U(1) but not imposing custodial symmetry. By not imposing custodial symmetry, there are two terms that we can write which are invariant only under SU$(2)\times$U(1). In order to see how to write it, recall that $\Sigma$ transforms as:
\[
...
%=\Tr{\Sigma^\dagger D_\mu\Sigma\sigma_3\Sigma^\dagger D_\mu\Sigma\sigma_3}
\marginnote{The $\sigma_3$ matrix is required otherwise the second term would be equal 0.}
\]
% The other kinetic term comes from by noticing that there has to be invariance under SU$(2)_{\text{L}}\times$U$(1)_{\text{Y}}$, but not invariance under SU$(2)_{\text{R}}$ which contains U$(1)_{\text{Y}}$. We can also write a double trace operator:
% \[
...
Summarizing, there is an explicit breaking from the masses, which is a small effect, and a spontaneous breaking from quark condensate which gives 3 NGBs resulting from the 3 broken generators. These particles in the spectrum are the pions, with a mass much smaller than $\Lambda_{\text{QCD}}$\marginnote{$m_{\pi^\pm}=139$\;MeV and $m_{\pi^0}=135$\;MeV.}.\\
...
% $\text{U}(1)_{\text{B}}$ is also included, the hypercharge is given by $Y=T_{3R}+(B-L)/2$.
...
In the standard situation, where $v\gg f_\pi$, this translates in the fact that the pions, which are the uneaten NGBs, acquire a mass both because of the quark masses and also because there is a radiative corrections coming from the gauge bosons\marginnote{
This is the reason why the mass of the charged pion is larger than the one of the neutral pion, the difference comes from this effect.}. If the hierarchy between $v$ and $f_\pi$ gets switched, the NGBs which are eaten will be mostly virtual and the real ones will get a mass from this effect. 
\[
\left\{
\begin{aligned}
...
However, here we made the brave choice to inflict a good amount of physical pain to ourselves and describe everything all at once: there is a large global symmetry which gives two sets of NGBs and by switching on the gauging we obtain linear combinations. This has the advantage that it is possible to switch the hierarchy in any way we like and at this point we do the calculation of the masses for the $W$ and the $Z$, neglecting electroweak symmetry breaking and focusing only on QCD\marginnote[-1cm]{This will end up to be qualitatively the right method but with the only problem than $f_\pi$ is ten thousand times smaller than the electroweak scale so quantitatively we will get masses that are ten thousand times smaller than the observed ones, but there will still be custodial symmetry protecting $\rho=1$.}.\\
...
\]
\marginnote[-2cm]{We can verify this by taking an axial current $A_\mu$ which excites a pion that then gets reabsorbed by $A_\nu$. In each vertex the loop is given by the vacuum expectation value between the axial current and the pion field. 
\[
...
\end{array}\right)
\]
% Here, for simplicity, we fixed $a=3$ and in this case there are both left and right. For $a=1,2$ there is no right, $g'=0$ and there is only $g^2$.
...
% The values are not quite correct because we should run the coupling $g$ down to the QCD scale, in this calculation the value of $g$ at a hundred GeV is used.
\section{Technicolor}
As anticipated, the masses we just found are too small but qualitatively everything is fine so this gives us an idea: \raisebox{-\mydepth}{{\includegraphics[height=1.1\baselineskip]{Images/lamp.png}}}\\ replicating this story by saying that the fundamental Higgs field which gives the electroweak symmetry breaking at 246 GeV is a copy of QCD. Why should we do this perverse complication? Because this UV dynamics does not have a fundamental scalar inside and fundamental scalars imply hierarchy problems.
% Such fundamental scalar has to be much lighter than this UV dynamics but at the moment we do not know the reason why it is like that.
...
This goes under the name of \textbf{technicolor}: take the SM, remove the Higgs and replace it with the QCD part. The orthogonal combinations of the uneaten NGBs remain in the spectrum plus the \nth{4} degree of freedom, which does not play the usual role of the Higgs boson. These 4 degrees of freedom are pretty light, the Higgs has its own usual mass $\mu^2=\lambda^2v^2$ but with a smaller $v$. The other three guys, the $\chi$, get a mass given by gauge interactions and Yukawa terms, which are small effects. By changing parameter, the part of electroweak symmetry breaking now is coming from QCD dynamics and qualitatively it works, so it is possible to think of technicolor\marginnote{This was before the Higgs discovery.}.\\
% The other important thing is that a little bit of symmetry breaking comes from the QCD sector but it is so small that is experimentally unobservable , the pions we see in the spectrum are not entirely bound states $q\overline{q}$ but there is a little bit of $\chi$ from the Higgs sector.
...
\begin{itemize}
    ...
    % The scattering amplitude should not be computed in terms of these bound states but rather in terms of techni-quarks degrees of freedom.
    The experimental signature is the growth of the $WW$ cross section until the theory becomes non perturbative.
    % There is no Higgs boson in this story, so we expect a very broad resonance.
...
The decay width $\Gamma_Z$ of \refeq{Zwidthtree} gets modified as:
\[
...
In particular, we saw the expression of $A_{\text{FB}}$ in terms of $A_e$ and $A_f$ [\refeq{AeAf}] with the latter given by a tree-level expression which now has to be modified:
\[
...
\subsection{Experimental Results}\marginnote{From \cite{lep}.}
...
&\hat{S}:=g^2[\Pi_{3B}'(0)-\Pi_{3B}'^{\text{SM}}(0)]\\
...
\left\{
\begin{aligned}
&\hat{S}:=16\pi[\Pi'_{3B}(0)-\Pi_{3B}'^{\text{SM}}(0)]\\
&\hat{T}:=\frac{4\pi}{s^2c^2m_Z^2}\left\{[\Pi_{WW}(0)-\Pi_{33}(0)]-[\Pi_{WW}^{\text{SM}}(0)-\Pi_{33}^{\text{SM}}(0)]\right\}\\
&\hat{U}:=16\pi\left\{[\Pi'_{WW}(0)-\Pi'_{33}(0)]-[\Pi_{WW}'^{\text{SM}}(0)-\Pi_{33}'^{\text{SM}}(0)]\right\}
\end{aligned}
\right.
\]
...
\begin{align*}
\frac{i}{q^2-m_Z^2+\frac{q^4\hat{W}}{m_Z^2}}&\simeq\frac{i}{q^2-m_Z^2}\left(1-\frac{q^4\hat{W}}{m_Z^2}\frac{1}{q^2-m_Z^2}\right)[1+\pazocal{O}(\hat{W})]\\
